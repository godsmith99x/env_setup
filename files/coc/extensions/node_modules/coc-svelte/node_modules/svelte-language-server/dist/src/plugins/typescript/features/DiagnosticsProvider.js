...
(function (DiagnosticCode) {
    ...
    DiagnosticCode[DiagnosticCode["MISSING_PROP"] = 2741] = "MISSING_PROP"; // "Property '..' is missing in type '..' but required in type '..'."
...
class DiagnosticsProviderImpl {
    ...
    async getDiagnostics(document, cancellationToken) {
        ...
        return diagnostics
            ...
            .map(mapRange(fragment, document, this.configManager.getConfig().svelte.useNewTransformation))
...
function mapRange(fragment, document, useNewTransformation) {
    return (diagnostic) => {
        var _a;
        let range = (0, documents_1.mapRangeToOriginal)(fragment, diagnostic.range);
        if (range.start.line < 0) {
            const is$$PropsError = (0, utils_2.isAfterSvelte2TsxPropsReturn)(fragment.text, fragment.offsetAt(diagnostic.range.start)) && diagnostic.message.includes('$$Props');
            if (is$$PropsError) {
                const propsStart = (0, utils_3.regexIndexOf)(document.getText(), /(interface|type)\s+\$\$Props[\s{=]/);
                if (propsStart) {
